...
const sketch = () => {
    return ({context, width, height, frame}) => {
        ...
        for (let i = 0; i < numCells; i++) {
            ...
            let a = w * 0.5;
            let b = h * 0.5;
            context.arc(a, b, params.radius, 0, 2 * Math.PI);
            context.strokeStyle = gradient;
            context.lineWidth = params.circleLineScale;
            context.stroke();
...
const createPane = () => {
    ...
    folder = pane.addFolder({title: 'ShapeControl'});
    folder.addInput(params, 'scaleMin', {min: 1, max: 100});
    folder.addInput(params, 'scaleMax', {min: 1, max: 100});
    folder.addInput(params, 'gradient', {min: 1, max: 180});
    folder.addInput(params, 'circleLineScale', {min: 1, max: 100});
    folder.addInput(params, 'radius', {min: 1, max: 50});
    folder.addInput(params, 'rectSize', {min: 0.1, max: 2});
